...
public class CreateFormF3 {
	public static void  generateResource(String[] strField,HttpServletRequest request)
	{
		...
		try{
			...
			ArrayList<String> lstTypeEnValDis6=new ArrayList<String>();
			ArrayList<String> lstLabelEnValDis6=new ArrayList<String>();
			ArrayList<String> lstTypeEnKeyDis6=new ArrayList<String>();//<msf_typeEnDis>
			ArrayList<String> lstLabelEnKeyDis6=new ArrayList<String>();//<msf_lableEnDis>
			
			ArrayList<String> lstTypeEnValDis7=new ArrayList<String>();
			ArrayList<String> lstLabelEnValDis7=new ArrayList<String>();
			ArrayList<String> lstTypeEnKeyDis7=new ArrayList<String>();//<msf_typeEnDis>
			ArrayList<String> lstLabelEnKeyDis7=new ArrayList<String>();//<msf_lableEnDis>
			...
			for(int i=0;i<strField.length;i++)
			{
				...
				//dis 6
				if(strField[i].contains("<msf_typeEnDis6>"))
				{
					lstTypeEnKeyDis6.add(strField[i]);
					lstTypeEnValDis6.add(request.getParameter(strField[i]));
				}
				
				if(strField[i].contains("<msf_labelEnDis6>"))
				{
					lstLabelEnKeyDis6.add(strField[i]);
					lstLabelEnValDis6.add(request.getParameter(strField[i]));
				}
				
				//dis 7
				if(strField[i].contains("<msf_typeEnDis7>"))
				{
					lstTypeEnKeyDis7.add(strField[i]);
					lstTypeEnValDis7.add(request.getParameter(strField[i]));
				}
				
				if(strField[i].contains("<msf_labelEnDis7>"))
				{
					lstLabelEnKeyDis7.add(strField[i]);
					lstLabelEnValDis7.add(request.getParameter(strField[i]));
				}
			...
			//dis6
			strOut=new StringBuilder();
			if(lstTypeEnKeyDis6.size()>0){
				for(int i=1;i<lstTypeEnKeyDis6.size();i++)
				{
					strOut.append("\n"+lstTypeEnKeyDis6.get(i).replace("<msf_typeEnDis6>", "")+"\t"+lstTypeEnValDis6.get(i));
				}
				if("<msf_typeEnDis6><File>".compareToIgnoreCase(lstTypeEnKeyDis6.get(0).trim())==0)
					CommonCode.doWrite(dirResource.getPath(),lstTypeEnValDis6.get(0), strOut.toString().trim());
				
			}
			
			strOut=new StringBuilder();
			if(lstLabelEnKeyDis6.size()>0){
				for(int i=1;i<lstLabelEnKeyDis6.size();i++)
				{
					strOut.append("\n"+lstLabelEnKeyDis6.get(i).replace("<msf_labelEnDis6>", "")+"\t"+lstLabelEnValDis6.get(i)+"\t"+strLang);
				}
				if("<msf_labelEnDis6><File>".compareToIgnoreCase(lstLabelEnKeyDis6.get(0).trim())==0)
					CommonCode.doWriteMultiLang(srcPath,dirResource.getPath(),lstLabelEnValDis6.get(0), strOut.toString().trim());
			...
			//dis7
			strOut=new StringBuilder();
			if(lstTypeEnKeyDis7.size()>0){
				for(int i=1;i<lstTypeEnKeyDis7.size();i++)
				{
					strOut.append("\n"+lstTypeEnKeyDis7.get(i).replace("<msf_typeEnDis7>", "")+"\t"+lstTypeEnValDis7.get(i));
				}
				if("<msf_typeEnDis7><File>".compareToIgnoreCase(lstTypeEnKeyDis7.get(0).trim())==0)
					CommonCode.doWrite(dirResource.getPath(),lstTypeEnValDis7.get(0), strOut.toString().trim());
				
			}
			
			strOut=new StringBuilder();
			if(lstLabelEnKeyDis7.size()>0){
				for(int i=1;i<lstLabelEnKeyDis7.size();i++)
				{
					strOut.append("\n"+lstLabelEnKeyDis7.get(i).replace("<msf_labelEnDis7>", "")+"\t"+lstLabelEnValDis7.get(i)+"\t"+strLang);
				}
				if("<msf_labelEnDis7><File>".compareToIgnoreCase(lstLabelEnKeyDis7.get(0).trim())==0)
					CommonCode.doWriteMultiLang(srcPath,dirResource.getPath(),lstLabelEnValDis7.get(0), strOut.toString().trim());
			...
			session.setAttribute("encounter6",request.getParameter("encounter6").toString());
			session.setAttribute("encounter7",request.getParameter("encounter7").toString());
	...
	public static void generateProperties(File dirResource, HttpSession session)
	{
        ...
        if("1".compareToIgnoreCase(session.getAttribute("encounter1").toString().trim())==0)
        	strFile.append("\nEncounterType-neonatology " + counter++);
        
        if("1".compareToIgnoreCase(session.getAttribute("encounter2").toString().trim())==0)
        	strFile.append("\nEncounterType-paediatrics " + counter++);
        
        if("1".compareToIgnoreCase(session.getAttribute("encounter3").toString().trim())==0)
        	strFile.append("\nEncounterType-medicine " + counter++);
        
        if("1".compareToIgnoreCase(session.getAttribute("encounter4").toString().trim())==0)
        	strFile.append("\nEncounterType-delivery " + counter++);
        
        if("1".compareToIgnoreCase(session.getAttribute("encounter5").toString().trim())==0)
        	strFile.append("\nEncounterType-abortion " + counter++);
        
        if("1".compareToIgnoreCase(session.getAttribute("encounter6").toString().trim())==0)
        	strFile.append("\nEncounterType-emergency_room " + counter++);
        
        if("1".compareToIgnoreCase(session.getAttribute("encounter7").toString().trim())==0)
        	strFile.append("\nEncounterType-surgery " + counter++);
