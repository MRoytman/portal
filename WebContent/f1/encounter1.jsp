...
<%@page contentType="text/html" pageEncoding="UTF-8"%>
 <%
	File dirEncounterFiles1=new File(session.getServletContext().getRealPath("/f1/Files_encounters/"));
	if (!dirEncounterFiles1.exists()) {
		dirEncounterFiles1.mkdirs();
		out.print("First time: need to copy the resources.");
	}else{
			String encounterPath1 = session.getServletContext().getRealPath("/f1/Files_encounters/");
		 	String fileNameTypeDis1="EncounterEpidemics_opd-FieldsIdType.txt";
		 	String fileNameLabelDis1="EncounterEpidemics_opd-FieldsLabel.txt";
		 	String strTitle="Case in OPD";//read from encounterLabel.txt
            ...
            while((lineTypeDis1 = rdTypeDis1.readLine())!= null){
        	   	...
        	   	try{
	        		...
	        		if(lineLabelDis1.endsWith(language))
	        		{
           ...
           if(dirResources != null && dirResources.length() != 0){// Edit form
               ...
               if(file.exists()){
				   ...
				   while((strLine = bufferedReader.readLine())!= null){
				       if (strLine.contains("Epidemics_opd")) {
				           isChecked = true;
				           break;
				       }
				   }
