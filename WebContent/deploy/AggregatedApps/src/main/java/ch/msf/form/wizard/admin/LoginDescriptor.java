...
public class LoginDescriptor extends WizardPanelDescriptor implements ActionListener {
	...
	public LoginDescriptor() {
		...
		_AdminUser = _ConfigurationManager.getConfigField("adminUser");
		_AdminUserPwd = _ConfigurationManager.getConfigField("adminUserPwd");

		System.out.println("---------" + _AdminUserPwd + "---------");
	...
	public void handleLogin() {
		...
		if (checkUserInput()) {
			boolean configUrgence = false;
			if (configUrgence) { // TN75
				getWizard().setCurrentPanel(AdminParamCountriesDescriptor.IDENTIFIER, true);
			}
			getWizard().setCurrentPanel(AdminParamDescriptor.IDENTIFIER, true);
		} else {
	...
	private boolean checkUserInput() {
		...
		if (_LoginPanel.getUserName().equals(_AdminUser) && _LoginPanel.getPassword().equals(_AdminUserPwd))
			return true;
