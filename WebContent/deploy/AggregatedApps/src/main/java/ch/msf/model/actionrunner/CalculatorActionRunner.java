 ...
 * 
 * It compute a total from other cells
 * 
 * @author cmi
 * 
 */

public class CalculatorActionRunner extends AbstractActionRuleRunnerBase {
...
//		System.out.println(getClass().getName()+" "+this.hashCode());
	...
	public boolean runAction(String command, Object[] runtimeParams) {
		...
		// ServiceHelper.getEntryFormConfigurationManagerService();
		//
		// // get our component to update
		// ComponentCellUpdator componentCellUpdator =
		// _ConfigurationManager.getComponentUpdatorCache().get(_GraphicalComponent);
		//
		// //
		// if (componentCellUpdator == null)
		// throw new ConfigException("_GraphicalComponent not found!:"+_GraphicalComponent);
		// //
		// componentCellUpdator.updateCell(thisNotifierId, allConditionMet);
	...
	private boolean doUpdate(String sourceConceptId, int total) {
		...
		if (componentCellUpdator == null) {
			...
			// throw new ConfigException("_GraphicalComponent not found!:" + _GraphicalComponent);
			return false; // TN9 do NOT throw exception because the cache might not be completely filled with GraphicalComponents
		}
		//
		if (componentCellUpdator != null) {
