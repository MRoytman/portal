...
public class MSFForm implements Runnable, CheckBusinessRulesChecker {
	...
	public boolean init() {
		...
		System.out.println(getClass().getName() + "::init............................javaVersion = " + javaVersion);
		...
		if (appIconName == null) {
			...
			getProperties().put("modeJavaWebStart", false);
		...
		if (!(this instanceof DataEntryFormI)) {//TN14 if admin
		...
		// TN13 check if file exists in the 'well known' directory
		...
		// ...load config file
		// TN13 at this point the configFileName should point to the project directory
		try {
	...
	/**
	 * TN13 set the config file path to the project one override project configuration file with configFilePathName
	 * 
	...
	private void overrideConfigurationFile(/* String configFilePathName */) {
		...
		if (source.exists()) {
			System.out.println(getClass().getName() + "overriding Configuration File::" + projectConfigFilePathName+ " with "
					+ overridingConfigFilePathName );
	...
	/**
	 * TN109 if entryFormConfig.xml exists, copy it to the right dir
	 */
	...
	// doValidateAnswerAction();
	// }
	// }
	//
	// private void doValidateAnswerAction() {
	// System.out.println("doValidateAnswerAction DOING NOTHING!");
	// }
	//
