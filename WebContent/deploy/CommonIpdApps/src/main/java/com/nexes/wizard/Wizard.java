...
public class Wizard extends WindowAdapter implements PropertyChangeListener {
	...
	// // CM my add
	// private CheckBusinessRulesChecker _CheckBusinessRulesChecker;
	...
	public Boolean _MyDirectionNext; // my modif the navigation direction
	...
	public int showModalDialog() {
		...
		// CM: my add (circumvent the fact that clicking on the window cross
		// does not close the windows)
		// TN125 (EXIT_ON_CLOSE close console when exiting application)
		wizardDialog.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);

		// wizardDialog.setModal(true);
		wizardDialog.pack();
		wizardDialog.setVisible(true); // show();
	 ...
	 */
	public void setCurrentPanel(Object id, boolean myDirection) {

		_MyDirectionNext = myDirection; // if true -> next
	...
	public void propertyChange(PropertyChangeEvent evt) {
		...
		} else if (evt.getPropertyName().equals(WizardModel.QUIT_BUTTON_TEXT_PROPERTY)) {
	...
	void close(int code) {
		...
		// check if admin
		boolean isAdmin = ServiceHelper.getDbManagerService().isAdmin();
		if (isAdmin) {
			boolean configurationSaved = configurationManager.isConfigurationSaved();
			if (!configurationSaved) { //
				String errMess = ServiceHelper.getMessageService().getMessage(CommonConstants.MESSAGEADMIN_CONFIG_NOT_SAVED);
			...
			String errMess = ServiceHelper.getMessageService().getMessage(CommonConstants.MESSAGE_CONFIRM_QUIT);
			retCode = JOptionPane.showOptionDialog(null, errMess, "Warning", JOptionPane.DEFAULT_OPTION, JOptionPane.WARNING_MESSAGE, null, options, options[1]);
		}
		if (retCode == 0) {
	...
	private void initComponents() {
		...
		buttonBox.setBorder(new EmptyBorder(new Insets(3, 5, 3, 5)));
	...
	private static Object getImage(String name, Class c) {
	...
	//
	// BACK_ICON = new ImageIcon((URL) getImage((String) (resources.getObject("backButtonIcon")), Wizard.class));
	// NEXT_ICON = new ImageIcon((URL) getImage((String) (resources.getObject("nextButtonIcon")), Wizard.class));
	// // CANCEL_ICON = new ImageIcon((URL) getImage((String) (resources.getObject("cancelButtonIcon")),
	// // Wizard.class));
	// FINISH_ICON = new ImageIcon((URL) getImage((String) (resources.getObject("finishButtonIcon")), Wizard.class));
	//
	// } catch (MissingResourceException mre) {
	// mre.printStackTrace();
	// throw new FatalException(StackTraceUtil.getCustomStackTrace(mre));
	// // System.exit(1);
	// }
	// }

	static PropertyResourceBundle s_Resources = null;
	static {
		try {
			ConfigurationManager configurationManager = ServiceHelper.getConfigurationManagerService();
			setDefaultLocale(configurationManager.getDefaultLanguage());

			s_Resources = (PropertyResourceBundle) ResourceBundle.getBundle("com.nexes.wizard.wizard");
			// // Locale locale_FR = new Locale("fr","FR");
			// PropertyResourceBundle resources = (PropertyResourceBundle)
			// ResourceBundle.getBundle("com.nexes.wizard.wizard", locale_FR);
	...
	private void loadButtons() {
		...
		BACK_TEXT = (String) (s_Resources.getObject("backButtonText"));

		NEXT_TEXT = (String) (s_Resources.getObject("nextButtonText"));
		// CANCEL_TEXT = (String) (resources.getObject("cancelButtonText"));
		FINISH_TEXT = (String) (s_Resources.getObject("finishButtonText"));

		BACK_ICON = new ImageIcon((URL) getImage((String) (s_Resources.getObject("backButtonIcon")), Wizard.class));
		NEXT_ICON = new ImageIcon((URL) getImage((String) (s_Resources.getObject("nextButtonIcon")), Wizard.class));
		// CANCEL_ICON = new ImageIcon((URL) getImage((String) (resources.getObject("cancelButtonIcon")),
		// Wizard.class));
		FINISH_ICON = new ImageIcon((URL) getImage((String) (s_Resources.getObject("finishButtonIcon")), Wizard.class));
	}
	...
	public void setLocale(String locale) {
		...
		// s_Resources.clearCache();

		s_Resources = (PropertyResourceBundle) ResourceBundle.getBundle("com.nexes.wizard.wizard");
		// s_Resources = (PropertyResourceBundle) ResourceBundle.getBundle("com.nexes.wizard.wizard", newLocale);
		loadButtons();
		// update dialog with new loaded resources
		wizardController.resetButtonsToPanelRules();
	}

	// TN135
	private static Locale setDefaultLocale(String locale) {
