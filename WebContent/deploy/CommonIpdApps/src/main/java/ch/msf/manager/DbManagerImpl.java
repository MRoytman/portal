...
public class DbManagerImpl implements DbManager {
	...
	@Override
	// TN132 manage roll back
	public void endTransaction(EntityManager em, boolean problem) {
		if (problem && em.getTransaction().isActive())
	...
	public EntityManager startTransaction() {
		...
		if (_Factory == null) {
			...
			String url = H2Base.getDBUrl(isAdmin);
			...
			if (isAdmin) {
				...
				_Factory = Persistence.createEntityManagerFactory("IpdOpdAdmin", configOverrides);

				// _Factory = Persistence
				// .createEntityManagerFactory("IpdOpdAdmin");
			} else {

				_Factory = Persistence.createEntityManagerFactory("IpdOpd", configOverrides);
				// _Factory = Persistence.createEntityManagerFactory("IpdOpd");
			}
		}
