...
public class ExportCSVf3 {
	
	String atFolder="/ch/msf/fields/f3/";
	private String strFilePat = "Patient-FieldsIdType.txt";
	private String strFileEn1 = "Encounterneonatology-FieldsIdType.txt";
	private String strFileEn2 = "Encounterpaediatrics-FieldsIdType.txt";
	private String strFileEn3 = "Encountermedicine-FieldsIdType.txt";
	private String strFileEn4 = "Encounterdelivery-FieldsIdType.txt";
	private String strFileEn5 = "Encounterabortion-FieldsIdType.txt";
	private String strFileEn6 = "Encounteremergency_room-FieldsIdType.txt";
	private String strFileEn7 = "Encountersurgery-FieldsIdType.txt";
	...
	List<String> strCodeFullEn6 = rf.readFile(strFileEn6,atFolder);
	List<String> strCodeFullEn7 = rf.readFile(strFileEn7,atFolder);
	...
	List<String> valPrintEn6Detail = null;
	List<String> valPrintEn7Detail = null;
			...
			for (int i = 0; i < lstPatientDB.size(); i++) {
				...
				for (int j = 0; j < lstContextDB.size(); j++) {
					...
					for (int k = 0; k < lstEncounterDB.size(); k++) {
						...
						if  (enType.compareToIgnoreCase(getCodeDB(strFileEn6))==0) {
							initLstDB();
							for (int l = 0; l < lstEncounterDetailDB.size(); l++) {
							...
							valPrintEn6Detail = sortValueDependOn(strCodeFullEn6);//mapping col and val in DB with header
						}
						if  (enType.compareToIgnoreCase(getCodeDB(strFileEn7))==0) {
							initLstDB();
							for (int l = 0; l < lstEncounterDetailDB.size(); l++) {
							...
							valPrintEn7Detail = sortValueDependOn(strCodeFullEn7);//mapping col and val in DB with header
						}
					...
					if(valPrintEn6Detail!=null)
						printCol(writer, valPrintEn6Detail);
					else
						printNull(writer,strCodeFullEn6.size());
					
					if(valPrintEn7Detail!=null)
						printCol(writer, valPrintEn7Detail);
					else
						printNull(writer,strCodeFullEn7.size());
					
					valPrintEn1Detail=valPrintEn2Detail=valPrintEn3Detail=valPrintEn4Detail=valPrintEn5Detail=valPrintEn6Detail=valPrintEn7Detail=null;
					writer.append('\n');
				}
			}
	...
	private void printHeader(FileWriter writer) throws IOException {
		...
		for (String string : strCodeFullEn6) {
			writer.append(getPrefix(strFileEn5) + string);
			writer.append(',');
		}
		for (String string : strCodeFullEn7) {
			writer.append(getPrefix(strFileEn5) + string);
			writer.append(',');
		}
		writer.append('\n');
