...
public class ParamPanel extends AbstractWizardPanel implements ActionListener {
	...
	public JPanel getContentPanel() {
		...
		try {
			...
			//TN135 select default language
			for(int index = 0; index<_Languages.getItemCount();index++){
				if (_Languages.getItemAt(index).equals(_ConfigurationManager.getDefaultLanguage())){
					_Languages.setSelectedIndex(index);
					break;
				}
			}
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == _Languages) {
			...
			getDescriptor().getWizard().setLocale(_ConfigurationManager.getDefaultLanguage());//TN135
