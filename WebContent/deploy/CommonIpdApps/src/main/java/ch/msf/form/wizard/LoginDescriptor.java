...
public class LoginDescriptor extends WizardPanelDescriptor implements ActionListener {
	...
	private boolean checkUserInput() {
		...
		if (_ConfigurationManager.isUserPermissionsActive())
			return ServiceHelper.getPermissionManagerService().isAuthorized(_LoginPanel.getUserName(), _LoginPanel.getPassword());
		else if (_LoginPanel.getUserName().equals(_MsfUser) && _LoginPanel.getPassword().equals(_MsfUserPwd))
			return true;
