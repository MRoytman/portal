...
public class PatientKeyValueTableModel extends AbstractTableModel implements ComponentEnabler {

	public final static int COL_KEYS = 0;
	public final static int COL_VALUES = 1;
	...
	public PatientKeyValueTableModel(ArrayList<IdType> idTypes, HashMap<String, HashMap<String, String>> labels, HashMap<String, ArrayList<KeyValue>> comboboxValueMap) {
	 ...
	 */
	public void reBuildModel(Patient currentPatient) {
		_KeyValues = new ArrayList<DicoData>();

		if (getIdTypes() == null)
			return; // usual if types not known yet
		for (IdType idType : getIdTypes()) {
			// set key and fieldType
			DicoData keyValue = new DicoData(idType._Key, idType._Value, currentPatient);
	...
	public Object getValueAt(int row, int col) {
		try {
			if (col == 0) {
				...
				String label = _Labels.get(_ConfigurationManager.getDefaultLanguage()).get(fieldId);
	...
	public class DicoData {
		...
		public DicoData(String key, String fieldType, Object entity) {
			...
			case CommonIpdConstants.DATE_TYPE:
				Patient currentPatient = (Patient) entity;
				Date creationDate = null;
				if (currentPatient != null)
					creationDate = currentPatient.getCreationDate();
				if (creationDate == null) // patient not yet created
	...
	/**
	 * 
	 * @return the values of the tablemodel
	 */
	public HashMap<String, String> getResults() {
		...
		for (DicoData dicoData : getKeyValues()) {
			...
			if (dicoData._Value == null) {
				...
				case CommonIpdConstants.TIME_TYPE:
					// strValue = _Sdft.format(dicoData._Value);
