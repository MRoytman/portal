...
<%@page contentType="text/html" pageEncoding="UTF-8"%>
 <%
	...
	File dirEncounterFiles3=new File(session.getServletContext().getRealPath("/f4/Files_encounters/"));
	if (!dirEncounterFiles3.exists()) {
		dirEncounterFiles3.mkdirs();
		out.print("First time: need to copy the resources.");
	}else{
			String encounterPath1 = session.getServletContext().getRealPath("/f4/Files_encounters/");
		 	String fileNameTypeDis1="Encounterncd_tests_results-FieldsIdType.txt";
		 	String fileNameLabelDis1="Encounterncd_tests_results-FieldsLabel.txt";
		 	String strTitle="Investigation";//read from encounterLabel.txt
            ...
            if(dirResources != null && dirResources.length() != 0){// Edit form
                ...
                if(file.exists()){
                    ...
                    while((strLine = bufferedReader.readLine())!= null){
                        if (strLine.contains("ncd_tests_results")) {
                            isChecked = true;
                            break;
                        }
                    }
