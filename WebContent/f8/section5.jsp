...
<%@page contentType="text/html" pageEncoding="UTF-8"%>
 <%
	...
	String sectionTmp5 = session.getServletContext().getRealPath("/f8/Files_sections/");
	File dirSectionFiles5=new File(sectionTmp5);
	
	if (!dirSectionFiles5.exists()) {
		dirSectionFiles5.mkdirs();
		out.print("First time: need to copy the resources.");
			...
			String sectionPath1 = session.getServletContext().getRealPath("/f8/Files_sections/");
		 	String fileNameId="Section-opd_nutrition_screening-TablesIds.txt";
		 	String fileNameTitle="Section-opd_nutrition_screening-TablesLabels.txt";

		 	String strTitle="Malnutrition screening";//read from encounterLabel.txt
		 	String xmlTitle="<msf_tableAGG_5>";
		 	String xmlRow="<msf_rowAGG_5>";
           ...
           rdId.close();
%>
		<a  href="javascript:toggle('section5');">
			<h2><%=strTitle %></a>&nbsp;&nbsp;&nbsp;&nbsp;Select:&nbsp;<input type="checkbox" name="section5" value="1" checked><input type="hidden" name="section5" value="0"> 
			</h2> <div id="section5" style="display: none" href="javascript:toggle();">	
			<input type="hidden" name="fileSection5" value="<%=fileNameId %>"> 
<%           
           String[] sectionTable=sbTableId.toString().split("SectionSeparator\n");
%>
			<input type="hidden" name="TotalTableSection5" value="<%=sectionTable.length %>"> 
<%
           String sectionI;
           String[] iRow;
           String[] iCol;
