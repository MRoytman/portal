...
<%@page contentType="text/html" pageEncoding="UTF-8"%>
 <%
	...
	String sectionTmp3 = session.getServletContext().getRealPath("/f8/Files_sections/");
	File dirSectionFiles3=new File(sectionTmp3);
	
	if (!dirSectionFiles3.exists()) {
		dirSectionFiles3.mkdirs();
		out.print("First time: need to copy the resources.");
			...
			String sectionPath1 = session.getServletContext().getRealPath("/f8/Files_sections/");
		 	String fileNameId="Section-opd_consultations_referrals-TablesIds.txt";
		 	String fileNameTitle="Section-opd_consultations_referrals-TablesLabels.txt";

		 	String strTitle="Consultations & referrals";//read from encounterLabel.txt
		 	String xmlTitle="<msf_tableAGG_3>";
		 	String xmlRow="<msf_rowAGG_3>";
           ...
           rdId.close();
%>
		<a  href="javascript:toggle('section3');">
			<h2><%=strTitle %></a>&nbsp;&nbsp;&nbsp;&nbsp;Select:&nbsp;<input type="checkbox" name="section3" value="1" checked><input type="hidden" name="section3" value="0"> 
			</h2> <div id="section3" style="display: none" href="javascript:toggle();">	
			<input type="hidden" name="fileSection3" value="<%=fileNameId %>"> 
<%           
           String[] sectionTable=sbTableId.toString().split("SectionSeparator\n");
%>
			<input type="hidden" name="TotalTableSection3" value="<%=sectionTable.length %>"> 
<%
           String sectionI;
           String[] iRow;
           String[] iCol;
