...
<%@page contentType="text/html" pageEncoding="UTF-8"%>
 <%
	...
	String sectionTmp8 = session.getServletContext().getRealPath("/f8/Files_sections/");
	File dirSectionFiles8=new File(sectionTmp8);
	
	if (!dirSectionFiles8.exists()) {
		dirSectionFiles8.mkdirs();
		out.print("First time: need to copy the resources.");
			...
			String sectionPath1 = session.getServletContext().getRealPath("/f8/Files_sections/");
		 	String fileNameId="Section-opd_reproductive-TablesIds.txt";
		 	String fileNameTitle="Section-opd_reproductive-TablesLabels.txt";

		 	String strTitle="Reproductive health";//read from encounterLabel.txt
		 	String xmlTitle="<msf_tableAGG_8>";
		 	String xmlRow="<msf_rowAGG_8>";
           ...
           rdId.close();
%>
		<a  href="javascript:toggle('section8');">
			<h2><%=strTitle %></a>&nbsp;&nbsp;&nbsp;&nbsp;Select:&nbsp;<input type="checkbox" name="section8" value="1" checked><input type="hidden" name="section8" value="0"> 
			</h2> <div id="section8" style="display: none" href="javascript:toggle();">	
			<input type="hidden" name="fileSection8" value="<%=fileNameId %>"> 
<%           
           String[] sectionTable=sbTableId.toString().split("SectionSeparator\n");
%>
			<input type="hidden" name="TotalTableSection8" value="<%=sectionTable.length %>"> 
<%
           String sectionI;
           String[] iRow;
           String[] iCol;
