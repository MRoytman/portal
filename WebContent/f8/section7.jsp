...
<%@page contentType="text/html" pageEncoding="UTF-8"%>
 <%
	...
	String sectionTmp7 = session.getServletContext().getRealPath("/f8/Files_sections/");
	File dirSectionFiles7=new File(sectionTmp7);
	
	if (!dirSectionFiles7.exists()) {
		dirSectionFiles7.mkdirs();
		out.print("First time: need to copy the resources.");
			...
			String sectionPath1 = session.getServletContext().getRealPath("/f8/Files_sections/");
		 	String fileNameId="Section-opd_vaccination-TablesIds.txt";
		 	String fileNameTitle="Section-opd_vaccination-TablesLabels.txt";

		 	String strTitle="Vaccination";//read from encounterLabel.txt
		 	String xmlTitle="<msf_tableAGG_7>";
		 	String xmlRow="<msf_rowAGG_7>";
           ...
           rdId.close();
%>
		<a  href="javascript:toggle('section7');">
			<h2><%=strTitle %></a>&nbsp;&nbsp;&nbsp;&nbsp;Select:&nbsp;<input type="checkbox" name="section7" value="1" checked><input type="hidden" name="section7" value="0"> 
			</h2> <div id="section7" style="display: none" href="javascript:toggle();">	
			<input type="hidden" name="fileSection7" value="<%=fileNameId %>"> 
<%           
           String[] sectionTable=sbTableId.toString().split("SectionSeparator\n");
%>
			<input type="hidden" name="TotalTableSection7" value="<%=sectionTable.length %>"> 
<%
           String sectionI;
           String[] iRow;
           String[] iCol;
