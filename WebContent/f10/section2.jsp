...
<%@page contentType="text/html" pageEncoding="UTF-8"%>
 <%
	...
	String sectionTmp2 = session.getServletContext().getRealPath("/f10/Files_sections/");
	File dirSectionFiles2=new File(sectionTmp2);
	
	if (!dirSectionFiles2.exists()) {
		dirSectionFiles2.mkdirs();
		out.print("First time: need to copy the resources.");
	}else{
			String sectionPath1 = session.getServletContext().getRealPath("/f10/Files_sections/");
		 	String fileNameId="Section-ipd_itfc-TablesIds.txt";
		 	String fileNameTitle="Section-ipd_itfc-TablesLabels.txt";
		 	String strTitle="ITFC";//read from encounterLabel.txt
		 	String xmlTitle="<msf_tableAGG_2>";
		 	String xmlRow="<msf_rowAGG_2>";
           ...
           rdId.close();
%>
		<a  href="javascript:toggle('section2');">
			<h2><%=strTitle %></a>&nbsp;&nbsp;&nbsp;&nbsp;Select:&nbsp;<input type="checkbox" name="section2" value="1" checked><input type="hidden" name="section2" value="0"> 
			</h2> <div id="section2" style="display: none" href="javascript:toggle();">	
			<input type="hidden" name="fileSection2" value="<%=fileNameId %>"> 
<%           
           String[] sectionTable=sbTableId.toString().split("SectionSeparator\n");
%>
			<input type="hidden" name="TotalTableSection2" value="<%=sectionTable.length %>"> 
<%
           String sectionI;
           String[] iRow;
           String[] iCol;
