...
<%@page contentType="text/html" pageEncoding="UTF-8"%>
 <%
	...
	String sectionTmp1 = session.getServletContext().getRealPath("/f10/Files_sections/");
	File dirSectionFiles1=new File(sectionTmp1);
	
	if (!dirSectionFiles1.exists()) {
		dirSectionFiles1.mkdirs();
		out.print("First time: need to copy the resources.");
			...
			String sectionPath1 = session.getServletContext().getRealPath("/f10/Files_sections/");
		 	String fileNameId="Section-opd_atfc-TablesIds.txt";
		 	String fileNameTitle="Section-opd_atfc-TablesLabels.txt";
		 	String strTitle="ATFC";
		 	String xmlTitle="<msf_tableAGG_1>";
		 	String xmlRow="<msf_rowAGG_1>";
           ...
           rdId.close();
%>
		<a  href="javascript:toggle('section1');">
			<h2><%=strTitle %></a>&nbsp;&nbsp;&nbsp;&nbsp;Select:&nbsp;<input type="checkbox" name="section1" value="1" checked><input type="hidden" name="section1" value="0"> 
			</h2> <div id="section1" style="display: none" href="javascript:toggle();">	
			<input type="hidden" name="fileSection1" value="<%=fileNameId %>"> 
<%           
           String[] sectionTable=sbTableId.toString().split("SectionSeparator\n");
%>
			<input type="hidden" name="TotalTableSection1" value="<%=sectionTable.length %>"> 
<%
           String sectionI;
           String[] iRow;
           String[] iCol;
